    ...
    where TCommand : ResponseCommand<TResponseDto>
{
    ...
    protected virtual Result<TResponseDto> Failure(Error error)
    {
        return Result.Failure<TResponseDto>(error);
    }

    protected virtual Result<TResponseDto> Failure(List<Error> errors)
    {
        return Result.Failure<TResponseDto>(errors);
    }
}